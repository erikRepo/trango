//! Hebrew niqud ONNX model selection: an in-app folder browser, reusing
//! `app-window.slint`'s `FileListDialog` chrome — same pattern as
//! `model_picker.rs` (whisper models), but scoped to `.onnx` files and
//! without whisper.cpp's English/multilingual naming convention, which
//! doesn't apply here. Always produces an absolute path (a browsed
//! `std::fs::read_dir` entry's path is always absolute), unlike the
//! niqud model path's original plain-text Settings field, which silently
//! accepted a relative one that only resolved by accident depending on
//! trango's current working directory at launch.
...
/// A selectable niqud model file — one possible [`FolderEntry`].
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ModelFileEntry {
    /// Absolute path to the model file — saved to
    /// `config::TrangoConfig::niqud_model_path`.
    pub path: PathBuf,
    /// Displayed name — just the filename, unlike whisper models there's
    /// no naming convention worth calling out.
    pub name: String,
}

/// One row in the niqud model picker's listing. Mirrors
/// `model_picker::FolderEntry`.
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// then subfolders sorted by name, then `.onnx` files sorted by name.
...
pub fn list_folder_entries(folder: &Path) -> Vec<FolderEntry> {
    ...
    let dir_entries = match fs::read_dir(folder) {
        ...
        Err(err) => {
            tracing::warn!(?folder, %err, "failed to read niqud model folder");
            return entries;
        }
    };
    ...
    for dir_entry in dir_entries.filter_map(|entry| entry.ok()) {
        ...
        } else if file_type.is_file() && is_onnx_file(&path) {
            let name = path
                .file_name()
                .map(|name| name.to_string_lossy().into_owned())
...
/// Whether `path` has a (case-insensitive) `.onnx` extension.
...
/// The niqud model picker's starting folder: the configured model's
/// parent folder if it still exists, else the current working directory
/// — no autodiscovery convention exists for this model the way whisper's
/// `whisper.cpp/models` does, since there's exactly one well-known model.
...
pub fn open_dialog(window: &AppWindow, folder: &Path, entries: &[FolderEntry]) {
    window.set_niqud_model_picker_folder_label(folder.display().to_string().into());
    window.set_niqud_model_picker_rows(Rc::new(VecModel::from(dialog_rows(entries, -1))).into());
    window.set_niqud_model_picker_selected_index(-1);
    window.set_is_niqud_model_picker_dialog_open(true);
}
...
pub fn mark_selected(window: &AppWindow, entries: &[FolderEntry], selected_index: i32) {
    window.set_niqud_model_picker_rows(
...
mod tests {
    ...
    fn temp_dir(name: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!("trango-test-niqud-model-picker-{name}"));
    ...
    fn test_list_folder_entries_filters_sorts_and_lists_subfolders_first() {
        // Given: a temp folder with an .onnx file (uppercase extension),
        //        an unrelated file, and a subfolder, created out of order
        // When:  listing its contents
        // Then:  Up, then the subfolder, then only the .onnx file
        ...
        fs::write(dir.join("phonikud-1.0.int8.ONNX"), b"").expect("failed to write fixture file");
        fs::write(dir.join("tokenizer.json"), b"").expect("failed to write fixture file");
        ...
        assert_eq!(
            ...
            dir.join("phonikud-1.0.int8.ONNX")
        );
    ...
    fn test_default_start_folder_prefers_configured_models_parent() {
        // Given: a config whose niqud_model_path's parent folder exists
        ...
        let config = TrangoConfig {
            niqud_model_path: Some(dir.join("phonikud-1.0.int8.onnx")),
            ..Default::default()
        };
    ...
    fn test_default_start_folder_falls_back_to_cwd_when_nothing_configured() {
        // Given: a config with no niqud_model_path set at all
        // When:  picking the picker's starting folder
