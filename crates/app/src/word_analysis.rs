...
mod tests {
    ...
    /// CLI wrapper (`tools/niqud-cli/`) at `dir.join(name)` — mirrors
    /// `subtitle::generate`'s `write_fake_binary`. Used here (rather than
    /// the `FakeNiqudClient`/`FixedNiqudClient` trait doubles above) to
    /// exercise the *real* `niqud::PhonikudCliClient` end to end: real
    /// subprocess spawn, real stdout JSON parsing, real
    /// `niqud_to_pronunciation` transliteration — only the Ollama HTTP
    /// call is faked, since there's no live Ollama server in tests/CI.
    #[cfg(unix)]
    fn write_fake_niqud_cli(dir: &std::path::Path) -> PathBuf {
        use std::os::unix::fs::PermissionsExt;

        let script_path = dir.join("fake-niqud-cli.sh");
        std::fs::write(
            &script_path,
            "#!/bin/sh\nprintf '{\"words\":[{\"word\":\"שכב\",\"niqud\":\"שָׁכַב\"}]}'\n",
        )
        .expect("failed to write fake niqud CLI script");
        std::fs::set_permissions(&script_path, std::fs::Permissions::from_mode(0o755))
            .expect("failed to make fake niqud CLI script executable");
        script_path
    }

    #[test]
    #[cfg(unix)]
    fn test_spawn_batch_analyze_runs_the_real_niqud_cli_client_end_to_end() {
        // Given: a Hebrew cue, a fake Ollama client, and a REAL
        //        niqud::PhonikudCliClient pointed at a fake niqud CLI
        //        shell script (rather than a NiqudClient trait double)
        // When:  running the batch analysis
        // Then:  the cached pronunciation is the one
        //        niqud_to_pronunciation derives from the fake CLI's niqud
        //        output ("sha-khav") — proving the real subprocess,
        //        JSON-parsing, and transliteration code paths all work
        //        together, not just the trait boundary
        let cache_path = temp_cache_path("real-phonikud-cli-client-end-to-end");
        let dir = cache_path.parent().unwrap();
        let _ = std::fs::remove_file(&cache_path);
        let binary_path = write_fake_niqud_cli(dir);
        let client = RecordingClient {
            calls: std::sync::Arc::new(std::sync::Mutex::new(Vec::new())),
            fail_sentences: Vec::new(),
            ..Default::default()
        ...
        spawn_batch_analyze(
            AnalysisClients {
                ollama: client,
                niqud: niqud::PhonikudCliClient { binary_path },
            },
            "llama3.1:8b".to_string(),
            "English".to_string(),
            cues,
