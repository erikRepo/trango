...
| Key | Effect | Mode |
| --- | --- | --- |
| Space | Play/pause. In Sentence by sentence with a line in focus: play that line's span, then auto-pause at its end; press again to replay it from the start. Otherwise: plain play/pause toggle. | Both |
...
| Ctrl+T | Show/hide the translated line under the current sentence | Both |
| Ctrl+A | Look up a word-by-word translation for the current sentence | Both |
