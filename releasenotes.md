...
- `niqud_to_pronunciation` deterministically converts niqud (diacritized Hebrew) text into a hyphenated Latin pronunciation guide (e.g. שָׁכַב -> "sha-khav"), validated against real phonikud-onnx output — no LLM guessing involved
- `NiqudClient` trait and CLI-output JSON parsing (`crates/niqud`), mirroring `word-analysis`'s `OllamaClient` trait — the niqud CLI wrapper itself lands in a later step
- `PhonikudCliClient` runs a niqud CLI wrapper as a subprocess per sentence (`crates/niqud`), mirroring `subtitle::WhisperCliGenerator`'s external-process pattern — the actual `tools/niqud-cli/` wrapper script lands in a later step
- `tools/niqud-cli/niqud_cli.py`: the actual niqud CLI wrapper (Phonikud has no CLI of its own), pinned to CPU inference — not yet wired into the app
- Hebrew sentences' word-analysis pronunciation is now derived from niqud instead of Ollama's own guess: automatic (detected from script, no setting), with graceful fallback to Ollama's guess if the niqud CLI isn't installed or its word count doesn't line up with Ollama's. See `docs/src/usage/word-analysis.md` and `TRANGO_NIQUD_CLI_PATH` for installing it
- Integration test running the real `niqud::PhonikudCliClient` (real subprocess spawn, real stdout parsing, real transliteration) against a fake niqud CLI script through the actual `spawn_batch_analyze` pipeline, closing out the Hebrew niqud pronunciation feature
