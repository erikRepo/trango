...
- Word-analysis's Ollama prompt now gives Hebrew sentences explicit guidance to always split single-letter prefix particles (ו/ה/ב/כ/ל/מ/ש, e.g. לסרטים = ל + סרטים) into their own word entry with its own translation — both the pedagogically correct behavior for a learner and more consistent than Ollama's previous ad-hoc splitting, though niqud's own word-count-mismatch fallback can still apply for prefixed sentences until the niqud pipeline also splits at prefix boundaries (not done yet — see `docs/src/developer/specs.md`)
